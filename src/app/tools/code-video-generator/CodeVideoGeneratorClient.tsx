...
function buildPreviewDocument(project: CodeVideoProject) {
  const css = project.css.replace(/<\/style/gi, "<\\/style");
  const js = project.js.replace(/<\/script/gi, "<\\/script");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>html,body{min-height:100%}${css}</style></head><body>${project.html}<script>try{${js}}catch(error){console.warn("Darma preview",error)}<\/script></body></html>`;
}
...
function waitForVideoMetadata(video: HTMLVideoElement) {
  return new Promise<void>((resolve, reject) => {
    const timeout = window.setTimeout(() => reject(new Error("The captured tab did not become ready.")), 8000);
...
export default function CodeVideoGeneratorClient() {
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      if (nextElapsed >= timeline.totalDurationMs) {
  ...
  useEffect(() => {
    ...
    if (!editor) return;
    const line = Math.max(1, snapshot.project[snapshot.activeFile].split("\n").length);
    editor.revealLineInCenterIfOutsideViewport(line);
    editor.setPosition({ lineNumber: line, column: 9999 });
  }, [snapshot.activeFile, snapshot.project]);
  ...
  const readImportedFiles = async (files: FileList | File[]) => {
    ...
    try {
      ...
      setImportStatus({ kind: "success", message: `Loaded ${imported.length} supported source file${imported.length === 1 ? "" : "s"} locally.` });
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        crop: {
          ...
          width: Math.min(video.videoWidth, rect.width * scaleX),
          height: Math.min(video.videoHeight, rect.height * scaleY),
        },
      };
    } catch (error) {
  ...
  const recordVideo = async () => {
    ...
    try {
      ...
      const recorder = new MediaRecorder(canvasStream, { ...(mimeType ? { mimeType } : {}), videoBitsPerSecond: settings.format === "youtube" ? 8_000_000 : 6_000_000 });
