  ...
  return (
    <section className="mx-auto max-w-[var(--container-wide)] px-4 pb-16 sm:px-6 lg:px-8" aria-labelledby="ways-explorer-title">
      <Card padding="lg" className="mb-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><p className="font-mono text-xs font-bold uppercase tracking-[0.14em] text-[var(--color-primary-text-strong)]">Choose by the problem</p><h2 id="ways-explorer-title" className="mt-3 text-3xl font-black tracking-[-0.035em] text-[var(--color-text-primary)]">Understand the system before copying the ceremony.</h2><p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">Compare purpose, flow, roles, cadence, artifacts, strengths, risks, and signs of healthy use.</p></div><div className="flex flex-wrap gap-2">{NEEDS.map((need) => <Button key={need.label} size="sm" variant="outline" onClick={() => setQuery(need.query)}>{need.label}</Button>)}</div></div>
      ...
      {filtered.length ? <div className="darma-grid-balance grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filtered.map((way) => <WayCard key={way.slug} way={way} />)}</div> : <Card padding="lg" className="text-center"><h3 className="text-xl font-bold text-[var(--color-text-primary)]">No approach matches those filters</h3><p className="mt-2 text-sm text-[var(--color-text-secondary)]">Reset the filters or search for the team problem you are trying to solve.</p><Button className="mt-5" variant="secondary" onClick={reset}>Show all approaches</Button></Card>}
    </section>
  );
}
