...
export default async function CareerPage({ params }: Props) {
  ...
  return <div className="pb-16"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(data).replace(/</g, "\\u003c") }} />
    ...
    <section id="responsibilities" className="mx-auto max-w-[var(--container-wide)] scroll-mt-32 px-4 py-10 sm:px-6 lg:px-8"><div className="grid gap-5 lg:grid-cols-2"><Card padding="lg"><h2 className="text-2xl font-black text-[var(--color-text-primary)]">A typical day may include</h2><ul className="mt-5 space-y-3">{career.typicalDay.map((item) => <li key={item} className="flex items-start gap-2 text-sm leading-6 text-[var(--color-text-secondary)]"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[var(--color-primary-text-strong)]" aria-hidden />{item}</li>)}</ul></Card><Card padding="lg"><h2 className="text-2xl font-black text-[var(--color-text-primary)]">Core responsibilities</h2><ul className="mt-5 space-y-3">{career.responsibilities.map((item) => <li key={item} className="flex items-start gap-2 text-sm leading-6 text-[var(--color-text-secondary)]"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[var(--color-success-text)]" aria-hidden />{item}</li>)}</ul></Card></div></section>

    <section id="skills" className="mx-auto max-w-[var(--container-wide)] scroll-mt-32 px-4 pb-10 sm:px-6 lg:px-8"><div className="grid gap-5 xl:grid-cols-3"><Card padding="lg"><Wrench className="h-5 w-5 text-[var(--color-primary-text-strong)]" aria-hidden /><h2 className="mt-4 text-xl font-black text-[var(--color-text-primary)]">Technical or professional skills</h2><ul className="mt-4 space-y-2">{career.skills.technical.map((skill) => <li key={skill} className="text-sm leading-6 text-[var(--color-text-secondary)]">• {skill}</li>)}</ul></Card><Card padding="lg"><Network className="h-5 w-5 text-[var(--color-primary-text-strong)]" aria-hidden /><h2 className="mt-4 text-xl font-black text-[var(--color-text-primary)]">Human skills</h2><ul className="mt-4 space-y-2">{career.skills.human.map((skill) => <li key={skill} className="text-sm leading-6 text-[var(--color-text-secondary)]">• {skill}</li>)}</ul></Card><Card padding="lg"><Sparkles className="h-5 w-5 text-[var(--color-primary-text-strong)]" aria-hidden /><h2 className="mt-4 text-xl font-black text-[var(--color-text-primary)]">Common outputs and tools</h2><p className="mt-4 font-mono text-xs font-bold uppercase tracking-[0.12em] text-[var(--color-text-tertiary)]">Deliverables</p><ul className="mt-2 space-y-1">{career.deliverables.map((item) => <li key={item} className="text-sm text-[var(--color-text-secondary)]">• {item}</li>)}</ul><p className="mt-4 font-mono text-xs font-bold uppercase tracking-[0.12em] text-[var(--color-text-tertiary)]">Common tools</p><div className="mt-2 flex flex-wrap gap-2">{career.tools.map((tool) => <Badge key={tool} variant="outline">{tool}</Badge>)}</div></Card></div></section>

    <section id="scope" className="mx-auto max-w-[var(--container-wide)] scroll-mt-32 px-4 pb-10 sm:px-6 lg:px-8"><div className="mb-5"><Badge variant="soft">Responsibility grows with scope</Badge><h2 className="mt-3 text-3xl font-black text-[var(--color-text-primary)]">How the scope grows</h2></div><div className="grid gap-5 lg:grid-cols-3">{Object.entries(career.levels).map(([key, level]) => <Card key={key} padding="lg"><Badge variant={key === "senior" ? "success" : "outline"}>{level.label}</Badge><p className="mt-4 text-sm leading-7 text-[var(--color-text-secondary)]">{level.scope}</p><ul className="mt-4 space-y-2">{level.evidence.map((item) => <li key={item} className="flex items-start gap-2 text-sm text-[var(--color-text-primary)]"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[var(--color-primary-text-strong)]" aria-hidden />{item}</li>)}</ul></Card>)}</div></section>

    <section id="start" className="mx-auto max-w-[var(--container-wide)] scroll-mt-28 px-4 pb-10 sm:px-6 lg:px-8"><div className="grid gap-5 lg:grid-cols-2"><Card padding="lg" className="border-[var(--color-primary-border)]"><GraduationCap className="h-5 w-5 text-[var(--color-primary-text-strong)]" aria-hidden /><h2 className="mt-4 text-2xl font-black text-[var(--color-text-primary)]">How to start</h2><ol className="mt-5 space-y-4">{career.howToStart.map((step, index) => <li key={step} className="flex items-start gap-3 text-sm leading-6 text-[var(--color-text-secondary)]"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary)] font-mono text-xs font-bold text-[var(--color-primary-text)]">{index + 1}</span>{step}</li>)}</ol></Card><Card padding="lg"><ShieldAlert className="h-5 w-5 text-[var(--color-warning-text)]" aria-hidden /><h2 className="mt-4 text-2xl font-black text-[var(--color-text-primary)]">Common misconceptions</h2><ul className="mt-5 space-y-4">{career.misconceptions.map((item) => <li key={item} className="rounded-[var(--radius-md)] border border-[var(--color-warning-border)] bg-[var(--color-warning-bg)] p-4 text-sm leading-6 text-[var(--color-text-primary)]">{item}</li>)}</ul></Card></div></section>

    {paths.length || resources.length ? <section id="role-resources" className="mx-auto max-w-[var(--container-wide)] scroll-mt-32 px-4 pb-10 sm:px-6 lg:px-8"><div className={`grid gap-5 ${paths.length && resources.length ? "lg:grid-cols-2" : ""}`}>{paths.length ? <Card padding="lg"><h2 className="text-2xl font-black text-[var(--color-text-primary)]">Learning paths that support this role</h2><div className="mt-5 space-y-3">{paths.map((path) => path ? <Link key={path.slug} href={`/learning-paths/${path.slug}`} className="flex items-center justify-between gap-3 rounded-[var(--radius-md)] border border-[var(--color-border-default)] p-4 text-sm font-bold text-[var(--color-text-primary)] transition hover:border-[var(--color-primary-border)] hover:text-[var(--color-primary-text-strong)]">{path.title}<ArrowRight className="h-4 w-4" aria-hidden /></Link> : null)}</div></Card> : null}{resources.length ? <Card padding="lg"><h2 className="text-2xl font-black text-[var(--color-text-primary)]">Starting references</h2><div className="mt-5 space-y-3">{resources.map((resource) => <Link key={resource.id} href={resource.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between gap-3 rounded-[var(--radius-md)] border border-[var(--color-border-default)] p-4 text-sm font-bold text-[var(--color-text-primary)] transition hover:border-[var(--color-primary-border)] hover:text-[var(--color-primary-text-strong)]">{resource.name}<ExternalLink className="h-4 w-4" aria-hidden /></Link>)}</div></Card> : null}</div></section> : null}
