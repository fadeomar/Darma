...
export function OutputPanel({
  ...
  return (
    <section aria-labelledby="output-panel-title" className="rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] p-3">
      <div>
        <h2 id="output-panel-title" className="text-sm font-black text-[var(--color-text-primary)]">Export</h2>
        <p className="mt-0.5 text-xs text-[var(--color-text-tertiary)]">Copy the CSS or download the current shape.</p>
      </div>

      {!validShape ? <p className="mt-3 text-xs text-[var(--color-danger-text)]">Fix the blocking polygon validation errors to enable exports.</p> : null}

      <div className="mt-3 grid grid-cols-2 gap-2">
        <CopyButton size="sm" text={validShape ? cssOutput : ""} disabled={!validShape}>Copy CSS</CopyButton>
