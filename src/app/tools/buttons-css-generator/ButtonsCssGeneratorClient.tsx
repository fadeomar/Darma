    ...
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 py-3 text-left [&::-webkit-details-marker]:hidden">
        <div className="min-w-0">
          <h3 className="font-mono text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--color-text-secondary)]">{title}</h3>
          {description ? <p className="mt-1 text-[11px] leading-4 text-[var(--color-text-tertiary)]">{description}</p> : null}
        </div>
...
function ContextPreview({ context, config, state }: { context: PreviewContext; config: ButtonGeneratorConfig; state: ButtonVisualState }) {
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-left shadow-xl">
      <div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Checkout</div><h3 className="mt-1 text-xl font-black tracking-[-0.035em] text-slate-950">Upgrade your workspace</h3></div><span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-black uppercase text-emerald-700">Ready</span></div>
...
function DevicePreviewFrame({
  ...
  return (
    <div className="flex w-full items-center justify-center p-4 sm:p-6">
      ...
      >
        {framed ? (
          <div className="flex h-9 items-center justify-between border-b border-slate-200 bg-slate-50 px-3 text-[10px] font-bold text-slate-500">
...
.button-studio-reduced .${name}__spinner { animation: none !important; }`).join("\n");
  ...
  function useFamilyMember(member: ButtonFamilyMember) {
    commitConfig({
      ...member.config,
      ...
      >
        <div className="flex flex-col gap-2">
          <div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
            <SegmentedControl ariaLabel="Preview state" value={previewState} onChange={setPreviewState} options={previewStates.map((state) => ({ value: state, label: state }))} />
            <SegmentedControl ariaLabel="Preview background" value={previewBackground} onChange={setPreviewBackground} options={backgroundOptions} />
          </div>
          <div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex items-center gap-2">
              <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">Device</span>
              <SegmentedControl ariaLabel="Preview device" value={previewDevice} onChange={setPreviewDevice} options={deviceOptions} />
            </div>
            <div className="flex items-center gap-2">
              <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">Input</span>
              <SegmentedControl ariaLabel="Input simulation" value={previewInput} onChange={changePreviewInput} options={inputOptions} />
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">Motion</span>
            <SegmentedControl ariaLabel="Motion simulation" value={motionPreview} onChange={setMotionPreview} options={[{ value: "normal", label: "Normal" }, { value: "reduced", label: "Reduced" }]} />
          </div>
          <p className="text-[11px] leading-5 text-[var(--color-text-tertiary)]">{interactionCopy(previewInput)}</p>
        </div>
      </PreviewToolbar>
      ...
      <div className={motionPreview === "reduced" ? "button-studio-reduced" : ""}>
          ...
          {comparePreviewConfig ? (
            <div className="grid gap-px bg-[var(--color-border-default)] lg:grid-cols-2">
              <div className="relative bg-[var(--color-surface-strong)]">
                <span className="absolute left-3 top-3 z-10 rounded-full border border-black/10 bg-white/90 px-2.5 py-1 font-mono text-[9px] font-black uppercase tracking-[0.08em] text-slate-700 shadow-sm">A · Baseline</span>
              ...
              <div className="relative bg-[var(--color-surface-strong)]">
                <span className="absolute left-3 top-3 z-10 rounded-full border border-black/10 bg-white/90 px-2.5 py-1 font-mono text-[9px] font-black uppercase tracking-[0.08em] text-slate-700 shadow-sm">B · Current</span>
                <DevicePreviewFrame device={previewDevice} surface={getPreviewSurface(previewBackground, customPreviewBackground)}>
          ...
          <div className="mx-4 mt-3 grid gap-2 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] p-3 sm:mx-5 sm:grid-cols-2 xl:grid-cols-4">
            {inspectionMetrics.map(([label, value]) => (
              <div key={label} className="rounded-[var(--radius-sm)] border border-[var(--color-border-subtle)] bg-[var(--color-surface)] px-3 py-2">
                <div className="font-mono text-[9px] font-black uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">{label}</div>
        ...
        {learnMode ? (
          <div className="mx-4 mt-3 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] p-3 sm:mx-5">
            <div className="mb-3"><div className="text-xs font-black text-[var(--color-text-primary)]">Learn what the controls generate</div><p className="mt-1 text-[11px] leading-5 text-[var(--color-text-tertiary)]">These snippets update live as you edit the button. They explain the CSS rather than replacing the full export below.</p></div>
            <div className="grid gap-2 md:grid-cols-2">
              {learningNotes.map((note) => (
                <div key={note.label} className="rounded-[var(--radius-sm)] border border-[var(--color-border-subtle)] bg-[var(--color-surface)] p-3">
                  <div className="font-mono text-[9px] font-black uppercase tracking-[0.08em] text-[var(--color-primary)]">{note.label}</div>
                  <code className="mt-2 block overflow-x-auto rounded bg-[var(--color-surface-inset)] px-2 py-1.5 text-[11px] text-[var(--color-text-primary)]">{note.css}</code>
                  <p className="mt-2 text-[11px] leading-5 text-[var(--color-text-tertiary)]">{note.explanation}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {comparePreviewConfig ? (
          <div className="mx-4 mt-3 flex flex-wrap items-center justify-between gap-2 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] px-3 py-2 sm:mx-5">
            ...
            <div className="flex gap-2"><Button size="sm" variant="secondary" onClick={useCompareA}>Use A</Button><Button size="sm" onClick={() => setCompareBaseline(null)}>Keep B</Button></div>
          </div>
        ) : null}
        ...
        <div className="mx-4 mt-4 flex flex-wrap gap-2 sm:mx-5">
          {productionMetrics.map((metric) => (
            <div key={metric.label} title={metric.detail} className={`flex items-center gap-2 rounded-full border px-3 py-1.5 shadow-[var(--shadow-xs)] ${toneClass(metric.tone)}`}>
              <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] opacity-70">{metric.label}</span>
          ...
          <div className="flex items-center gap-2 rounded-full border border-[var(--color-border-default)] bg-[var(--color-surface-strong)] px-3 py-1.5 text-[var(--color-text-secondary)] shadow-[var(--shadow-xs)]">
            <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] opacity-70">Simulation</span>
            <span className="text-xs font-black">{motionPreview === "reduced" ? "Reduced motion" : "Normal motion"}</span>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-[var(--color-border-default)] bg-[var(--color-surface-strong)] px-3 py-1.5 text-[var(--color-text-secondary)] shadow-[var(--shadow-xs)]">
            <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] opacity-70">Device</span>
            <span className="text-xs font-black">{previewDevice}</span>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-[var(--color-border-default)] bg-[var(--color-surface-strong)] px-3 py-1.5 text-[var(--color-text-secondary)] shadow-[var(--shadow-xs)]">
            <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] opacity-70">Input</span>
            <span className="text-xs font-black">{previewInput}</span>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-[var(--color-border-default)] bg-[var(--color-surface-strong)] px-3 py-1.5 text-[var(--color-text-secondary)] shadow-[var(--shadow-xs)]">
            <span className="font-mono text-[9px] font-black uppercase tracking-[0.08em] opacity-70">Output</span>
            ...
            {previewStates.map((state) => {
              ...
              return (
                <div key={state} className="rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface)] p-3">
                  <div className="mb-3 font-mono text-[9px] font-black uppercase tracking-[0.1em] text-[var(--color-text-tertiary)]">{state}</div>
        ...
        <section className="mx-4 mt-4 overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] sm:mx-5">
          <div className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="font-mono text-[9px] font-black uppercase tracking-[0.1em] text-[var(--color-primary)]">Design system lab</div>
          ...
          {familyEnabled ? (
            <div className="border-t border-[var(--color-border-subtle)] p-3 sm:p-4">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <div><div className="text-xs font-black text-[var(--color-text-primary)]">Button family</div><div className="mt-1 text-[11px] text-[var(--color-text-tertiary)]">Six semantic roles inherit your radius, sizing, typography, motion, and accessibility defaults.</div></div>
                ...
                {buttonFamily.map((member) => (
                  <div key={member.role} className="overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface)]">
                    ...
                    <div className="border-t border-[var(--color-border-subtle)] p-3">
                      <div className="flex items-center justify-between gap-2"><span className="text-xs font-black text-[var(--color-text-primary)]">{member.label}</span><button type="button" onClick={() => useFamilyMember(member)} className="text-[11px] font-bold text-[var(--color-primary)] hover:underline">Use style</button></div>
                      <p className="mt-1 text-[10px] leading-4 text-[var(--color-text-tertiary)]">{member.description}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {themePairEnabled ? (
            <div className="border-t border-[var(--color-border-subtle)] p-3 sm:p-4">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <div><div className="text-xs font-black text-[var(--color-text-primary)]">Automatic theme pair</div><div className="mt-1 text-[11px] text-[var(--color-text-tertiary)]">The dark variant adjusts accent visibility, readable text, borders, shadows, and focus treatment for a dark surface.</div></div>
              ...
              <div className="grid overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] lg:grid-cols-2">
                <div className="flex min-h-44 flex-col items-center justify-center gap-3 bg-white p-6">
                  <span className="font-mono text-[9px] font-black uppercase tracking-[0.1em] text-slate-500">Light</span>
                ...
                <div className="flex min-h-44 flex-col items-center justify-center gap-3 bg-slate-950 p-6">
                  <span className="font-mono text-[9px] font-black uppercase tracking-[0.1em] text-slate-400">Dark</span>
    ...
    >
      ...
      <StudioSection title="States & motion" description="Design hover, press, focus, and motion without changing the default state." defaultOpen>
        ...
        <div className="rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] p-3">
          <div className="flex items-center justify-between gap-3"><div><div className="text-xs font-black text-[var(--color-text-primary)]">Custom hover state</div><div className="mt-1 text-[11px] leading-4 text-[var(--color-text-tertiary)]">Override preset motion with your own colors, lift, scale, and shadow.</div></div><input type="checkbox" checked={config.customizeHoverState} onChange={(event) => event.target.checked ? copyDefaultToHover() : patch({ customizeHoverState: false })} className="h-4 w-4 shrink-0 accent-[var(--color-accent)]" /></div>
      ...
      <StudioSection title="Accessibility & advanced" description="Production defaults, CSS import, and scoped overrides for power users.">
        ...
        <div className="rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] p-3">
          <div className="flex items-start gap-2"><Upload className="mt-0.5 h-4 w-4 shrink-0 text-[var(--color-primary)]" /><div><div className="text-xs font-black text-[var(--color-text-primary)]">Import existing CSS</div><p className="mt-1 text-[11px] leading-5 text-[var(--color-text-tertiary)]">Paste a normal button rule. Darma maps supported properties into editable controls and keeps unmatched base declarations as scoped custom CSS.</p></div></div>
          ...
          {cssImportFeedback ? <p role="status" aria-live="polite" className="mt-2 rounded-[var(--radius-sm)] bg-[var(--color-surface)] px-2.5 py-2 text-[11px] leading-5 text-[var(--color-text-secondary)]">{cssImportFeedback}</p> : null}
        </div>

        <div className="rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-strong)] p-3">
          <div className="text-xs font-black text-[var(--color-text-primary)]">Scoped custom CSS</div>
          <p className="mt-1 text-[11px] leading-5 text-[var(--color-text-tertiary)]">Declarations only. They are appended inside the generated button selector. Global rules, braces, URLs, and at-rules are stripped before preview/export.</p>
        ...
        <ToolLayoutVisualGenerator
          ...
          actionsSlot={
            <div className="flex w-full flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">{shareStatus ? <span role="status" aria-live="polite" className="max-w-56 text-right text-[11px] leading-4 text-[var(--color-text-tertiary)]">{shareStatus}</span> : null}<Button variant="secondary" onClick={copyShareLink} leftIcon={<Link2 className="h-4 w-4" />}>Share</Button><CopyButton text={css}>Copy CSS</CopyButton></div>
            </div>
          }
          codeSlot={
            <div className="space-y-4">
