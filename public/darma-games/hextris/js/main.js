...
function setBottomContainer() {
    var buttonOffsetSource = $("#buttonCont").offset() || $("#bottomContainer").offset();
    if (!buttonOffsetSource || !trueCanvas) return;
    var buttonOffset = buttonOffsetSource.top;
...
function set_score_pos() {
    ...
    var target = $("#buttonCont").offset() || $("#bottomContainer").offset();
    var igt = $("#highScoreInGameText");
    if (!container.length || !container.offset() || !target || !igt.length || !igt.offset()) return;
    container.css('margin-top', '0');
    var middle_of_container = (container.height()/2 + container.offset().top);
    var top_of_bottom_container = target.top;
...
function resumeGame() {
	if (window.DarmaHextrisAudio) window.DarmaHextrisAudio.play('start');
	gameState = 1;
	hideUIElements();
	$('#pauseBtn').show();
	$('#restartBtn').hide();
...
function init(b) {
	if (window.DarmaHextrisAudio && b) window.DarmaHextrisAudio.play('restart');
...
function animLoop() {
	...
	case 1:
		...
		if (checkGameOver() && !importing) {
			if (window.DarmaHextrisAudio) window.DarmaHextrisAudio.play('lose');
			var saveState = localStorage.getItem("saveState") || "{}";
			saveState = JSONfn.parse(saveState);
			gameState = 2;
...
function showHelp() {
	...
	$("#inst_main_body").html("<div id='instructions_head'>HOW TO PLAY</div><p>The goal of Hextris is to keep the falling blocks inside the outer hexagon.</p><p>" + (settings.platform != 'mobile' ? 'Use the left and right arrow keys, or A/D,' : 'Tap the left and right sides of the screen') + " to rotate the hexagon." + (settings.platform != 'mobile' ? ' Hold the down arrow or S to speed up falling blocks.' : '') + "</p><p>Clear blocks by connecting 3 or more blocks of the same color.</p><p>Your combo timer appears as colored lines around the outer hexagon. Keep clearing blocks quickly to keep the combo alive.</p><hr><p id='afterhr'>Built into Darma Games with local assets, Darma themes, and Darma sound controls only.</p>");
	if (gameState == 1) {
		pause();
	}
...
// Darma embedded build: external scripts removed.
