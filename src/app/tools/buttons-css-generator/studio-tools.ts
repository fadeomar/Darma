...
function parseBaseRule(entries: Array<[string, string]>, base: ButtonGeneratorConfig) {
  ...
  let matched = 0;
  const markUsed = (...properties: string[]) => properties.forEach((property) => consumed.add(property));
  ...
  if (gradient) {
    ...
    markUsed("background", "background-image");
    hit(3);
  } else {
  ...
  if (borderWidth !== null) { next.borderEnabled = true; next.borderWidth = Math.max(1, borderWidth); markUsed("border-width"); hit(); }
  if (["solid", "dashed", "dotted", "double"].includes(map["border-style"])) { next.borderEnabled = true; next.borderStyle = map["border-style"] as ButtonBorderStyle; markUsed("border-style"); hit(); }
  const borderColor = normalizeColor(map["border-color"]);
  if (borderColor && borderColor !== "transparent") { next.borderEnabled = true; next.borderColor = borderColor; markUsed("border-color"); hit(); }

  const fontSize = parsePx(map["font-size"]);
  if (fontSize !== null) { next.fontSize = fontSize; markUsed("font-size"); hit(); }
  const fontWeight = parseNumber(map["font-weight"]);
  if (fontWeight !== null) { next.fontWeight = Math.max(100, Math.min(900, Math.round(fontWeight / 100) * 100)); markUsed("font-weight"); hit(); }
  const lineHeight = parseNumber(map["line-height"]);
  if (lineHeight !== null) { next.lineHeight = lineHeight; markUsed("line-height"); hit(); }
  const letterSpacing = parsePx(map["letter-spacing"]);
  if (letterSpacing !== null) { next.letterSpacing = letterSpacing; markUsed("letter-spacing"); hit(); }

  const minWidth = parsePx(map["min-width"]);
  if (minWidth !== null) { next.minWidth = minWidth; markUsed("min-width"); hit(); }
  if (map.width?.trim() === "100%") { next.fullWidth = true; markUsed("width"); hit(); }
  if (map["text-transform"]?.toLowerCase() === "uppercase") { next.uppercase = true; markUsed("text-transform"); hit(); }
  if (map["text-transform"]?.toLowerCase() === "none") { next.uppercase = false; markUsed("text-transform"); hit(); }

  const shadow = parseShadow(map["box-shadow"]);
  if (map["box-shadow"]?.trim() === "none") { next.shadowEnabled = false; markUsed("box-shadow"); hit(); }
  else if (shadow) {
  ...
  if (duration) {
    ...
    markUsed("transition"); hit(easing ? 2 : 1);
  }
