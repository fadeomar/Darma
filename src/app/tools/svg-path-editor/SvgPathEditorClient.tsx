...
type ButtonProps = { children: ReactNode; onClick?: () => void; disabled?: boolean; title?: string; active?: boolean };
...
type ExampleCategory = "Shapes" | "Arrows" | "Icons" | "UI Elements" | "Decorative";
...
const EXAMPLES: Example[] = [
  ...
  { label: "Diamond", category: "Shapes", path: "M 50 5 L 95 50 L 50 95 L 5 50 Z" },
  { label: "Pentagon", category: "Shapes", path: "M 50 5 L 98 35 L 79 91 L 21 91 L 2 35 Z" },
  { label: "Hexagon", category: "Shapes", path: "M 50 3 L 97 25 L 97 75 L 50 97 L 3 75 L 3 25 Z" },
  { label: "Star 5pt", category: "Shapes", path: "M 50 5 L 61 35 L 95 35 L 68 57 L 79 91 L 50 70 L 21 91 L 32 57 L 5 35 L 39 35 Z" },
  ...
  { label: "Arrow up", category: "Arrows", path: "M 40 90 V 40 H 20 L 50 10 L 80 40 H 60 V 90 Z" },
  { label: "Double arrow", category: "Arrows", path: "M 20 50 L 40 30 L 40 45 H 60 V 30 L 80 50 L 60 70 V 55 H 40 V 70 Z" },
  ...
  { label: "Lightning", category: "Icons", path: "M 60 5 L 25 55 H 48 L 40 95 L 75 45 H 52 Z" },
  { label: "Shield", category: "Icons", path: "M 50 5 L 90 20 V 55 C 90 75 70 90 50 95 C 30 90 10 75 10 55 V 20 Z" },
  { label: "Home", category: "Icons", path: "M 50 10 L 90 50 H 75 V 90 H 60 V 65 H 40 V 90 H 25 V 50 H 10 Z" },
  ...
  { label: "Play button", category: "UI Elements", path: "M 50 5 A 45 45 0 1 0 50 95 A 45 45 0 0 0 50 5 Z M 38 30 L 72 50 L 38 70 Z" },
  { label: "Search", category: "UI Elements", path: "M 42 15 A 27 27 0 1 0 42 69 A 27 27 0 0 0 42 15 Z M 62 62 L 85 85" },
  ...
  { label: "Spiral", category: "Decorative", path: "M 50 50 C 50 35 65 30 70 40 C 75 50 65 62 50 62 C 35 62 22 48 25 33 C 28 18 45 10 60 15" },
  { label: "Leaf", category: "Decorative", path: "M 50 90 C 10 70 10 20 50 10 C 90 20 90 70 50 90 Z M 50 90 L 50 10" },
  { label: "Flame", category: "Decorative", path: "M 50 90 C 20 75 15 50 30 35 C 28 50 38 55 40 45 C 42 30 55 20 50 5 C 70 20 75 45 65 55 C 68 45 72 48 70 60 C 80 45 78 25 65 15 C 80 25 90 50 80 70 C 75 80 65 88 50 90 Z" },
...
function parsePath(value: string): ParseResult {
  ...
  if (!trimmed) return { path: null, error: "Paste or import an SVG path to start editing." };
  try {
...
function persistSaved(items: SavedPath[]) {
  try { localStorage.setItem(LS_SAVED_KEY, JSON.stringify(items)); } catch { /* noop */ }
}
...
function NumberField({
  ...
  label: string; value: number; step?: number; hint?: string; onChange: (value: number) => void;
}) {
  return (
...
function ExampleCard({ example, onLoad }: { example: Example; onLoad: (path: string) => void }) {
  ...
  return (
    <button type="button" className="svg-editor-example-card" onClick={() => onLoad(example.path)} title={`Load: ${example.label}`}>
      <div className="svg-editor-example-preview">
        <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
          <path d={preview} fill="none" stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
...
export default function SvgPathEditorClient() {
  ...
  // UI
  const [activeTab, setActiveTab] = useState<"controls" | "examples" | "saved">("controls");
  ...
  const filteredExamples = useMemo(() => {
    ...
    return EXAMPLES.filter((e) => {
      const matchCat = exampleCategory === "All" || e.category === exampleCategory;
      const matchQ = !q || e.label.toLowerCase().includes(q) || e.category.toLowerCase().includes(q);
  ...
  function showToast(msg: string) {
    ...
    setToast(msg);
    toastTimerRef.current = window.setTimeout(() => setToast(""), TOAST_DURATION);
  }
  ...
  function toggleSave() {
    ...
    if (isSaved) {
      ...
      const entry: SavedPath = { id: `${Date.now()}`, label: `Path ${savedPaths.length + 1}`, path: key, savedAt: Date.now() };
  ...
  function eventPoint(event: PointerEvent<SVGSVGElement>) {
    ...
    return new Point(round(snap ? Math.round(x / 10) * 10 : x), round(snap ? Math.round(y / 10) * 10 : y));
  }
  ...
  function onPointerDown(event: PointerEvent<SVGSVGElement>) {
    ...
    if (event.button === 1 || event.buttons === 4) {
      ...
      setPan({ startX: event.clientX, startY: event.clientY, boxStart: { ...box } });
  ...
  function onPointerMove(event: PointerEvent<SVGSVGElement>) {
    if (pan) {
      ...
      if (!el) return;
      const rect = el.getBoundingClientRect();
      const dx = ((event.clientX - pan.startX) / rect.width) * pan.boxStart.width;
      const dy = ((event.clientY - pan.startY) / rect.height) * pan.boxStart.height;
      setBox({ ...pan.boxStart, x: pan.boxStart.x - dx, y: pan.boxStart.y - dy });
      return;
    }
  ...
  function endDrag(event: PointerEvent<SVGSVGElement>) {
    if (pan) {
      setPan(null);
      try { (event.currentTarget as SVGSVGElement).releasePointerCapture(event.pointerId); } catch { /* noop */ }
      return;
    }
    if (!drag) return;
  ...
  const handleKeyDown = useCallback((e: globalThis.KeyboardEvent) => {
    ...
    if (e.key === "Escape") { setSelected(0); setHovered(null); return; }
    if (["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(e.key) && svg && !error) {
      e.preventDefault();
      ...
      run((path) => {
        ...
        path.setLocation(loc as SvgPoint, new Point(loc.x + dx, loc.y + dy));
      }, { keepSelection: true });
    }
  ...
  useEffect(() => {
    ...
    function onWheel(e: WheelEvent) {
      ...
      setBox((current) => {
        ...
        const svgX = current.x + ((e.clientX - rect.left) / rect.width) * current.width;
        const svgY = current.y + ((e.clientY - rect.top) / rect.height) * current.height;
        return {
  ...
  const grid = useMemo(() => {
    ...
    for (let x = Math.floor(box.x / step) * step; x <= box.x + box.width; x += step)
      lines.push(<line key={`x${x}`} x1={x} y1={box.y} x2={x} y2={box.y + box.height} />);
    for (let y = Math.floor(box.y / step) * step; y <= box.y + box.height; y += step)
      lines.push(<line key={`y${y}`} x1={box.x} y1={y} x2={box.x + box.width} y2={y} />);
    return lines;
  }, [showGrid, box]);
  ...
  return (
    <main className="svg-editor-page">
      ...
      {/* Shortcuts modal */}
      {showShortcuts && (
        <div className="svg-editor-modal-backdrop" onClick={() => setShowShortcuts(false)}>
          <div className="svg-editor-modal" onClick={(e) => e.stopPropagation()}>
            <div className="svg-editor-modal-header">
              <strong>Keyboard shortcuts</strong>
              <button type="button" className="svg-editor-modal-close" onClick={() => setShowShortcuts(false)}>✕</button>
            </div>
      ...
      <section className="svg-editor-topbar" aria-label="SVG editor quick actions">
        ...
        <div className="svg-editor-hero-actions">
          <Button onClick={undo} disabled={historyIndex <= 0} title="Undo (Ctrl+Z)"><Undo2 size={15} /> Undo</Button>
          <Button onClick={redo} disabled={historyIndex >= history.length - 1} title="Redo (Ctrl+Y)"><Redo2 size={15} /> Redo</Button>
          <Button onClick={toggleSave} active={isSaved} title={isSaved ? "Remove from saved" : "Save current path"}>
      ...
      <section className="svg-editor-workbench">
        ...
        <aside className="svg-editor-panel svg-editor-left-panel">
          <div className="svg-editor-tabs">
            <button type="button" className={activeTab === "controls" ? "is-active" : ""} onClick={() => setActiveTab("controls")}>Controls</button>
            <button type="button" className={activeTab === "examples" ? "is-active" : ""} onClick={() => setActiveTab("examples")}>
              <Star size={12} /> Examples
            </button>
            <button type="button" className={activeTab === "saved" ? "is-active" : ""} onClick={() => setActiveTab("saved")}>
              <Bookmark size={12} /> Saved{savedPaths.length > 0 && <span className="svg-editor-tab-badge">{savedPaths.length}</span>}
            </button>
          </div>
          ...
          {activeTab === "controls" && (
            <>
              <div className="svg-editor-section">
                <SectionTitle icon={<Code2 size={14} />}>Path input</SectionTitle>
                <textarea
                  className={error && rawPath ? "has-error" : ""}
                  value={rawPath}
                  onChange={(e) => setDraftPath(e.target.value)}
                  onBlur={() => { const p = parsePath(rawPath).path; if (p) commit(p.asString(decimals, minify)); }}
                  spellCheck={false}
                />
                {error && rawPath
                  ? <p className="svg-editor-error">{error}</p>
                  : <p className="svg-editor-help">✓ Valid · {svg?.path.length ?? 0} commands</p>}
              </div>

              <div className="svg-editor-section">
                <SectionTitle icon={<FileInput size={14} />}>Import SVG</SectionTitle>
                <textarea
                ...
                {importPaths.length > 1 && (
                  <div className="svg-editor-multi-path">
                    <p className="svg-editor-help">{importPaths.length} paths found — pick one:</p>
                    {importPaths.map((p, i) => (
                      <button key={i} type="button" className="svg-editor-multi-path-row" onClick={() => loadPath(p)}>
                        <span>Path {i + 1}</span>
                        <small>{p.slice(0, 42)}{p.length > 42 ? "…" : ""}</small>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="svg-editor-section">
                ...
                <div className="svg-editor-grid-two">
                  ...
                  <NumberField label="Y" value={box.y} onChange={(v) => setBox((c) => ({ ...c, y: v }))} />
                  <NumberField label="W" value={box.width} onChange={(v) => setBox((c) => ({ ...c, width: Math.max(10, v) }))} />
                  <NumberField label="H" value={box.height} onChange={(v) => setBox((c) => ({ ...c, height: Math.max(10, v) }))} />
                </div>
                <Button onClick={fitToPath} disabled={Boolean(error)}><ScanSearch size={14} /> Fit to path</Button>
              </div>
              ...
              <div className="svg-editor-section">
                ...
                <div className="svg-editor-transform-group">
                  ...
                  <div className="svg-editor-grid-two">
                    <NumberField label="X" value={scaleX} step={0.1} onChange={setScaleX} />
                    <NumberField label="Y" value={scaleY} step={0.1} onChange={setScaleY} />
                  </div>
                ...
                <div className="svg-editor-transform-group">
                  ...
                  <div className="svg-editor-grid-three">
                    <NumberField label="Origin X" value={rotateX} onChange={setRotateX} />
                    <NumberField label="Origin Y" value={rotateY} onChange={setRotateY} />
                    <NumberField label="Angle °" value={rotateAngle} onChange={setRotateAngle} />
                  </div>
                  <Button onClick={() => run((path) => path.rotate(rotateX, rotateY, rotateAngle))} disabled={Boolean(error)}>Apply rotate</Button>
                </div>
                <div className="svg-editor-transform-group">
                  <p className="svg-editor-group-label">Output precision</p>
                  <NumberField label="Decimals (0–6)" value={decimals} onChange={(v) => setDecimals(Math.max(0, Math.min(6, Math.round(v))))} />
              ...
              <div className="svg-editor-section">
                ...
                <div className="svg-editor-button-grid">
                  ...
                  <Button onClick={() => commit(DEFAULT_PATH)}><RotateCcw size={13} /> Reset</Button>
                  <Button onClick={() => setDraftPath("")}><Eraser size={13} /> Clear</Button>
                </div>
                <div className="svg-editor-toggles">
                  <label className="svg-editor-toggle"><input type="checkbox" checked={minify} onChange={(e) => setMinify(e.target.checked)} /><span>Minify output</span></label>
                  <label className="svg-editor-toggle"><input type="checkbox" checked={fillPreview} onChange={(e) => setFillPreview(e.target.checked)} /><span>Fill preview</span></label>
                </div>
              </div>

              <div className="svg-editor-section">
                <SectionTitle icon={<Palette size={14} />}>Canvas style</SectionTitle>
                <div className="svg-editor-grid-two">
          ...
          {activeTab === "examples" && (
            <div className="svg-editor-examples-panel">
              <div className="svg-editor-search-wrap">
                <Search size={14} />
                <input type="text" placeholder="Search examples…" value={exampleSearch} onChange={(e) => setExampleSearch(e.target.value)} />
              </div>
              <div className="svg-editor-cat-pills">
                {(["All", ...EXAMPLE_CATEGORIES] as const).map((cat) => (
                  <button key={cat} type="button" className={exampleCategory === cat ? "is-active" : ""} onClick={() => setExampleCategory(cat)}>{cat}</button>
                ))}
              </div>
              {filteredExamples.length === 0
                ? <p className="svg-editor-help" style={{ padding: "var(--space-4) var(--space-3)" }}>No examples match.</p>
                : (
                  <div className="svg-editor-examples-grid">
                    {filteredExamples.map((ex) => (
                      <ExampleCard key={ex.label} example={ex} onLoad={(path) => {
                        commit(path);
              ...
              {savedPaths.length === 0
                ? (
                  <div className="svg-editor-saved-empty">
                    ...
                    <p className="svg-editor-help">Hit <strong>Save</strong> in the toolbar to bookmark the current path.</p>
                  </div>
                )
                : savedPaths.map((s) => (
                  <div key={s.id} className="svg-editor-saved-card">
                    <div className="svg-editor-saved-preview" onClick={() => { loadPath(s.path); setActiveTab("controls"); }}>
                      <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                        <path d={s.path} fill="none" stroke="currentColor" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                    <div className="svg-editor-saved-meta">
                      <input className="svg-editor-saved-name" value={s.label} onChange={(e) => renameSaved(s.id, e.target.value)} />
                      <p className="svg-editor-saved-path">{s.path.slice(0, 50)}{s.path.length > 50 ? "…" : ""}</p>
                      <div className="svg-editor-saved-actions">
                        <button type="button" onClick={() => { loadPath(s.path); setActiveTab("controls"); }}>Load</button>
                        <button type="button" onClick={() => copy("Copied", s.path)}>Copy</button>
                        <button type="button" className="is-danger" onClick={() => deleteSaved(s.id)}>Delete</button>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <section className="svg-editor-canvas-card">
          <div className="svg-editor-canvas-toolbar">
            ...
            <div className="svg-editor-canvas-actions">
              ...
              <Button onClick={fitToPath} title="Fit to path" disabled={Boolean(error)}><ScanSearch size={15} /></Button>
              <Button onClick={() => setBox({ x: 0, y: 0, width: 580, height: 460 })}>Reset</Button>
              <Button onClick={() => setShowGrid((v) => !v)} active={showGrid}><Grid3X3 size={15} /></Button>
          ...
          <div className="svg-editor-canvas-wrap">
            {tooltip && (
              <div className="svg-editor-point-tooltip" style={{ left: tooltip.x, top: tooltip.y }}>
                {round(tooltip.cx, 2)}, {round(tooltip.cy, 2)}
              </div>
            )}
            <div className="svg-editor-canvas-hint">Scroll to zoom · Middle-drag to pan</div>
            <svg
              ref={svgRef}
              ...
              {!error && svg ? (
                <>
                  ...
                  {hovered !== null && svg.path[hovered]
                    ? <path d={svg.path[hovered].asStandaloneString()} className="svg-editor-hover-path" />
                    : null}
                  {selectedItem
                    ? <path d={selectedItem.asStandaloneString()} className="svg-editor-selected-path" />
                    : null}
                  {controls.map((control, index) => (
                    <g key={`c-${index}`}>
                      {control.relations.map((relation, ri) => (
                        <line key={ri} x1={control.x} y1={control.y} x2={relation.x} y2={relation.y} className="svg-editor-handle" />
                      ))}
                      <circle
                        cx={control.x} cy={control.y} r={5}
                        className="svg-editor-control-point"
                        onPointerEnter={(e) => {
                          const rect = svgRef.current?.getBoundingClientRect();
                          if (rect) setTooltip({ x: e.clientX - rect.left + 12, y: e.clientY - rect.top - 32, cx: control.x, cy: control.y });
                        }}
                        onPointerLeave={() => setTooltip(null)}
                        onPointerDown={(e) => { e.stopPropagation(); setDrag({ kind: "control", index }); e.currentTarget.setPointerCapture(e.pointerId); }}
                      />
                    </g>
                  ))}
                  {targets.map((target, index) => (
                    <circle
                      ...
                      onPointerEnter={(e) => {
                        setHovered(index);
                        const rect = svgRef.current?.getBoundingClientRect();
                        if (rect) setTooltip({ x: e.clientX - rect.left + 12, y: e.clientY - rect.top - 32, cx: target.x, cy: target.y });
                      }}
                      onPointerLeave={() => { setHovered(null); setTooltip(null); }}
                      onPointerDown={(e) => { e.stopPropagation(); setSelected(index); setDrag({ kind: "target", index }); e.currentTarget.setPointerCapture(e.pointerId); }}
                    />
                  ))}
                </>
              ) : (
        ...
        <aside className="svg-editor-panel svg-editor-right-panel">
          <div className="svg-editor-section">
            <SectionTitle icon={<Scissors size={14} />}>Selected command</SectionTitle>
            {selectedItem ? (
              <>
                <div className="svg-editor-command-summary">
                  <strong>{selectedItem.getType()}</strong>
                  <span>#{selectedIndex + 1} · {round(selectedItem.targetLocation().x)}, {round(selectedItem.targetLocation().y)}</span>
                </div>
                <div className="svg-editor-grid-two">
                ...
                <div className="svg-editor-command-actions">
                  <select value={selectedItem.getType(true)} onChange={(e) => run((path) => path.changeType(path.path[selectedIndex], e.target.value as SvgCommandTypeAny), { keepSelection: true })} disabled={selectedIndex === 0}>
                  ...
                  <Button onClick={() => { const p = selectedItem.targetLocation(); run((path) => path.insert(SvgItem.Make(["L", String(p.x + 40), String(p.y)]), path.path[selectedIndex])); setSelected((v) => v + 1); }}>Insert L</Button>
                  <Button onClick={() => { run((path) => path.delete(path.path[selectedIndex])); setSelected((v) => Math.max(0, v - 1)); }} disabled={selectedIndex === 0}>Delete</Button>
                </div>
              </>
            ) : <p className="svg-editor-help">Click a point on the canvas or a command below to inspect it.</p>}
          </div>
          ...
          <div className="svg-editor-section">
            ...
            <div className="svg-editor-command-list">
              {svg?.path.map((item, index) => (
                ...
                >
                  <span className="svg-editor-command-badge">{item.getType()}</span>
