define(['base64', 'app/entity/building', 'app/entity/block', 'app/eventmanager', 'app/gamecontent'], 
	...
	var STORAGE_PREFIX = "darma:games:gridland:v1:";
	function getSlotKey(slot) { return STORAGE_PREFIX + "slot" + slot; }
	...
	var GameState = {
		...
		getExportCode: function(slot) {
			try {
				var savedState = localStorage[getSlotKey(slot)];
		...
		getSlotInfo: function(slot) {
			try {
				var savedState = JSON.parse(localStorage[getSlotKey(slot)]);
				if(savedState) {
					return {
		...
		save: function() {
			if(typeof Storage != 'undefined' && localStorage) {
				...
				localStorage[getSlotKey(loadedSlot)] = JSON.stringify(state);
			}
			return this;
		},
		...
		import: function(slotNum, importCode) {
			try {
				localStorage[getSlotKey(slotNum)] = Base64.decode(importCode);
			} catch(e) {
				return null;
			}
		},
		...
		deleteSlot: function(slotNum) {
			if(typeof Storage != 'undefined' && localStorage) {
				localStorage.removeItem(getSlotKey(slotNum));
			}
		},
		...
		savePersistents: function() {
			if(typeof Storage != 'undefined' && localStorage && localStorage[getSlotKey(loadedSlot)]) {
				var savedState = JSON.parse(localStorage[getSlotKey(loadedSlot)]);
				savedState.counts = this.counts;
				savedState.health = this.health;
				localStorage[getSlotKey(loadedSlot)] = JSON.stringify(savedState);
			}
		},
