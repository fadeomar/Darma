...
export function WarningPanel({ messages, title, className }: WarningPanelProps) {
  ...
  return (
    <section className={cn("space-y-2", className)} aria-live="polite">
      {title ? <h2 className="font-mono text-xs font-bold uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">{title}</h2> : null}
          ...
          return (
            <div key={item.id} className={cn("rounded-[var(--radius-md)] border px-3 py-2.5 shadow-[var(--shadow-xs)]", severityClass[severity])}>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-[var(--radius-full)] border border-current px-2 py-0.5 font-mono text-xs font-bold uppercase leading-none tracking-[0.08em] opacity-75">
