...
import { DEFAULT_LOAN_INPUT, DEFAULT_LOAN_OPTIONS, LOAN_PRESETS } from "./presets";
import type {
  CurrencyCode,
  LoanCheckLevel,
  LoanOptions,
...
function SummaryCard({ label, value, hint, icon }: { label: string; value: string; hint: string; icon: React.ReactNode }) {
  return <div className="min-w-0 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] px-3 py-2.5 shadow-[var(--shadow-xs)]"><div className="flex items-center justify-between gap-2"><span className="truncate font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">{label}</span><span className="text-[var(--color-primary)]">{icon}</span></div><div className="mt-1 truncate text-xl font-black tracking-tight text-[var(--color-text-primary)]" title={value}>{value}</div><div className="mt-0.5 truncate text-[11px] text-[var(--color-text-tertiary)]">{hint}</div></div>;
}
...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return <label className="block min-w-0"><span className="mb-1 flex items-center justify-between gap-2 text-[11px] font-bold text-[var(--color-text-secondary)]"><span>{label}</span>{hint ? <span className="font-normal text-[var(--color-text-tertiary)]">{hint}</span> : null}</span>{children}</label>;
}
...
function BalanceChart({ accelerated, baseline, startBalance, formatMoney }: { accelerated: Array<{ paymentNumber: number; closingBalance: number }>; baseline: Array<{ paymentNumber: number; closingBalance: number }>; startBalance: number; formatMoney: (value: number) => string }) {
  ...
  return <div>
    <div className="rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] p-3">
      ...
      <div className="mt-1 flex flex-wrap items-center justify-between gap-2 text-[10px] text-[var(--color-text-tertiary)]"><span>{formatMoney(maxBalance)} starting balance</span><span>{maxMonths} months</span></div>
    </div>
    <div className="mt-2 flex flex-wrap gap-4 text-[11px] text-[var(--color-text-secondary)]"><span className="flex items-center gap-1.5"><span className="h-0.5 w-5 bg-[var(--color-accent)]" />Entered plan</span><span className="flex items-center gap-1.5"><span className="h-0.5 w-5 border-t border-dashed border-[var(--color-text-tertiary)]" />No-extra baseline</span></div>
  </div>;
}
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_370px]">
          ...
          {result && baseline && comparison && activeTab === "overview" ? <div className="space-y-4 p-4">
            <div className="grid gap-3 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="rounded-[var(--radius-md)] border border-[var(--color-primary-border)] bg-[var(--color-primary-soft)] p-5">
                <div className="text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--color-primary)]">Scheduled monthly payment</div>
                ...
                {input.extraMonthlyPayment > 0 ? <div className="mt-3 inline-flex rounded-full border border-[var(--color-success-border)] bg-[var(--color-success-bg)] px-2.5 py-1 text-[11px] font-bold text-[var(--color-success-text)]">Planned monthly outflow: {formatMoney(result.scheduledMonthlyPayment + input.extraMonthlyPayment)}</div> : null}
              </div>
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
              <section><div className="mb-2 flex items-center justify-between gap-2"><div><h3 className="text-sm font-bold text-[var(--color-text-primary)]">Balance timeline</h3><p className="text-[11px] text-[var(--color-text-tertiary)]">Entered plan compared with the same loan without extra payments.</p></div><TrendingDown className="h-5 w-5 text-[var(--color-primary)]" /></div><BalanceChart accelerated={result.monthlySchedule} baseline={baseline.monthlySchedule} startBalance={result.financedPrincipal} formatMoney={formatMoney} /></section>
              <section className="space-y-3 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] p-4"><div className="flex items-center gap-2"><PiggyBank className="h-5 w-5 text-[var(--color-success-text)]" /><h3 className="text-sm font-bold text-[var(--color-text-primary)]">Cost breakdown</h3></div><div><div className="mb-1 flex justify-between text-[11px]"><span>Principal</span><span>{formatMoney(result.financedPrincipal)}</span></div><div className="flex h-3 overflow-hidden rounded-full bg-[var(--color-surface-subtle)]"><div className="bg-[var(--color-accent)]" style={{ width: `${100 - result.interestSharePercent}%` }} /><div className="bg-[var(--color-warning)]" style={{ width: `${result.interestSharePercent}%` }} /></div><div className="mt-1 flex justify-between text-[10px] text-[var(--color-text-tertiary)]"><span>{(100 - result.interestSharePercent).toFixed(1)}%</span><span>{result.interestSharePercent}% interest</span></div></div><div className="grid grid-cols-2 gap-2"><MetricCard label="Interest saved" value={formatMoney(comparison.interestSaved)} /><MetricCard label="Time saved" value={formatDuration(comparison.monthsSaved)} /></div><div className="rounded-[var(--radius-sm)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] p-2.5 text-[11px] leading-5 text-[var(--color-text-secondary)]">The rate entered is not necessarily the lender&apos;s APR. Financed fees and payment timing can change the disclosed APR and total cost.</div></section>
            </div>
          </div> : null}

          {result && activeTab === "monthly" ? <div>
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border-subtle)] px-4 py-3"><div><h3 className="text-sm font-bold text-[var(--color-text-primary)]">Monthly amortization</h3><p className="text-[11px] text-[var(--color-text-tertiary)]">Rows {schedulePage * rowsPerPage + 1}–{Math.min((schedulePage + 1) * rowsPerPage, result.payoffMonths)} of {result.payoffMonths}.</p></div><Button size="sm" variant="secondary" onClick={() => downloadText("monthly-amortization.csv", monthlyCsv, "text/csv;charset=utf-8")}><FileSpreadsheet className="h-4 w-4" />CSV</Button></div>
      ...
      <aside className="min-w-0 space-y-4">
        <section className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border-default)] bg-[var(--color-surface-overlay)] shadow-[var(--shadow-sm)]">
          <div className="border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-base)]/75 px-4 py-3"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-[var(--color-primary)]" /><h2 className="text-sm font-bold text-[var(--color-text-primary)]">Loan scenario</h2></div><p className="mt-0.5 text-[11px] text-[var(--color-text-tertiary)]">Fixed-rate monthly amortization with optional acceleration.</p></div>
          <div className="space-y-4 p-4">
            <Field label="Practical preset"><Select value="" onChange={(event) => applyPreset(event.target.value)}><option value="" disabled>Choose a scenario…</option>{LOAN_PRESETS.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</Select></Field>
