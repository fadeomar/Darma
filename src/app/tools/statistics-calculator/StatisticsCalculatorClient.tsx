...
function SummaryCard({ label, value, hint }: { label: string; value: string; hint: string }) {
  return <div className="min-w-0 rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] px-3 py-2.5 shadow-[var(--shadow-xs)]"><div className="truncate font-mono text-xs font-bold uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">{label}</div><div className="mt-1 truncate text-xl font-black tracking-tight text-[var(--color-text-primary)]" title={value}>{value}</div><div className="mt-0.5 truncate text-xs text-[var(--color-text-tertiary)]">{hint}</div></div>;
}
...
function HistogramChart({ bins, precision }: { bins: HistogramBin[]; precision: number }) {
  ...
  return <div>
    ...
    <div className="mt-1 flex justify-between font-mono text-xs text-[var(--color-text-tertiary)]"><span>{bins[0] ? formatStat(bins[0].start, precision) : "—"}</span><span>{bins.at(-1) ? formatStat(bins.at(-1)!.end, precision) : "—"}</span></div>
  </div>;
}
      ...
      <main className="min-w-0 space-y-4">
        <section className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border-default)] bg-[var(--color-surface-overlay)] shadow-[var(--shadow-sm)]">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-base)]/75 px-4 py-3">
            <div><h2 className="text-sm font-bold text-[var(--color-text-primary)]">Data set</h2><p className="text-xs text-[var(--color-text-tertiary)]">Commas, semicolons, pipes, spaces, tabs, and line breaks are supported.</p></div>
