...
export function WayCard({ way }: { way: WayOfWorking }) {
  return (
    <Card variant="interactive" padding="lg" className="flex h-full flex-col">
      ...
      <div className="mt-5 flex h-11 w-11 items-center justify-center rounded-[var(--radius-md)] bg-[var(--color-primary-soft)] text-[var(--color-primary)]"><GitBranch className="h-5 w-5" aria-hidden /></div>
      ...
      <Link href={`/ways-of-working/${way.slug}`} className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-[var(--color-primary)] transition hover:gap-3">Understand the workflow <ArrowRight className="h-4 w-4" aria-hidden /></Link>
    </Card>
  );
}
