  ...
  return (
    <div className="pb-20">
      ...
      <section className="visual-grid-bg border-b border-[var(--color-border-subtle)]">
        <div className="mx-auto grid max-w-[var(--container-wide)] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,1fr)_minmax(380px,.82fr)] lg:items-center lg:px-8 lg:py-20">
          <div>
            ...
            <SplitTextReveal text="Find a technology role worth exploring without treating it as a permanent label." className="darma-balanced-heading mt-5 text-4xl font-black tracking-[-0.05em] text-[var(--color-text-primary)] sm:text-5xl lg:text-6xl" />
            <p className="darma-pretty-copy mt-6 max-w-3xl text-base leading-8 text-[var(--color-text-secondary)] sm:text-lg">Use your preferred outcomes, craft, collaboration style, and environment to identify three practical starting directions. Then open the detailed role guides and test the fit through real work.</p>
      ...
      <MotionSection as="section" className="mx-auto max-w-[var(--container-page)] px-4 sm:px-6 lg:px-8" distance={18}>
        <Card padding="lg" className="grid gap-6 border-[var(--color-accent-border)] bg-[var(--color-accent-soft)] lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex items-center gap-2 text-[var(--color-accent-text)]"><CheckCircle2 className="h-5 w-5" aria-hidden /><span className="font-mono text-xs font-black uppercase tracking-[0.14em]">How to use the result</span></div><h2 className="mt-3 text-2xl font-black text-[var(--color-text-primary)]">Read two role guides, try one small project, then compare the experience.</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--color-text-secondary)]">A useful career decision comes from evidence: what work you can sustain, which problems you enjoy, the feedback you receive, and the environment where you perform well.</p></div><Link href="/guides/technology-careers-guide" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[var(--color-text-primary)] px-5 text-sm font-black text-[var(--color-surface-base)]">Read the career guide <ArrowRight className="h-4 w-4" aria-hidden /></Link></Card>
      </MotionSection>
    </div>
  );
}
