    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_330px]">
      <section className="min-w-0 overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] shadow-[var(--shadow-sm)]">
          ...
          <div className="space-y-3">
            {config.groups.map((group, groupIndex) => <article key={group.id} className="rounded-[var(--radius-md)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)]">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border-subtle)] px-3 py-2.5">
                <div className="min-w-0"><div className="truncate text-sm font-bold text-[var(--color-text-primary)]">Group {groupIndex + 1} · {groupLabel(group, groupIndex)}</div><div className="text-[11px] text-[var(--color-text-tertiary)]">Matching agents share the rules below.</div></div>
        ...
        {view === "preview" && <div className="grid gap-4 p-4 lg:grid-cols-2">
          <div className="min-w-0"><div className="mb-2 flex items-center justify-between gap-2"><div><h3 className="text-sm font-bold text-[var(--color-text-primary)]">robots.txt</h3><p className="text-[11px] text-[var(--color-text-tertiary)]">Upload at the exact site root.</p></div><CopyButton text={result.output} size="sm" variant="secondary">Copy</CopyButton></div><pre className="max-h-[590px] overflow-auto rounded-[var(--radius-md)] bg-slate-950 p-4 text-xs leading-6 text-slate-100"><code>{result.output}</code></pre></div>
          <div className="min-w-0"><div className="mb-2 flex items-center justify-between gap-2"><div><h3 className="text-sm font-bold text-[var(--color-text-primary)]">Next.js app/robots.ts</h3><p className="text-[11px] text-[var(--color-text-tertiary)]">MetadataRoute starter generated from the same policy.</p></div><CopyButton text={nextJsCode} size="sm" variant="secondary">Copy</CopyButton></div><pre className="max-h-[590px] overflow-auto rounded-[var(--radius-md)] bg-slate-950 p-4 text-xs leading-6 text-slate-100"><code>{nextJsCode}</code></pre></div>
        </div>}
      ...
      <aside className="space-y-3">
        <section className="rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] p-4 shadow-[var(--shadow-sm)]"><div className="mb-3 flex items-center gap-2 font-bold text-[var(--color-text-primary)]"><ListChecks className="h-4 w-4" />Practical presets</div><div className="space-y-2">{ROBOTS_PRESETS.map((preset) => <button key={preset.id} onClick={() => applyPreset(preset.id)} className={`w-full rounded-[var(--radius-sm)] border p-2.5 text-left transition hover:border-[var(--color-accent)] ${preset.destructive ? "border-[var(--color-danger-border)]" : "border-[var(--color-border-subtle)]"}`}><div className="text-sm font-bold text-[var(--color-text-primary)]">{preset.label}</div><div className="mt-0.5 text-[11px] leading-4 text-[var(--color-text-tertiary)]">{preset.description}</div></button>)}</div></section>

        <section className="space-y-3 rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-surface-base)] p-4 shadow-[var(--shadow-sm)]"><div className="flex items-center gap-2 font-bold text-[var(--color-text-primary)]"><FileSearch className="h-4 w-4" />Local route tester</div><label className="block text-xs font-semibold text-[var(--color-text-muted)]">Crawler product token<Input size="sm" className="mt-1 font-mono" value={testCrawler} onChange={(event) => setTestCrawler(event.target.value)} placeholder="Googlebot" /></label><div className="flex flex-wrap gap-1">{CRAWLER_TEST_PRESETS.map((crawler) => <button key={crawler} type="button" onClick={() => setTestCrawler(crawler)} className="rounded-full border border-[var(--color-border-subtle)] px-2 py-1 text-[10px] text-[var(--color-text-tertiary)] hover:border-[var(--color-accent)] hover:text-[var(--color-text-primary)]">{crawler}</button>)}</div><label className="block text-xs font-semibold text-[var(--color-text-muted)]">URL or path<Input size="sm" className="mt-1 font-mono" value={testPath} onChange={(event) => setTestPath(event.target.value)} /></label><div className={`rounded-[var(--radius-md)] border p-3 ${routeTest.allowed ? CHECK_STYLES.success : CHECK_STYLES.danger}`}><div className="flex items-center gap-2 font-bold">{routeTest.allowed ? <ShieldCheck className="h-4 w-4" /> : <ShieldAlert className="h-4 w-4" />}{routeTest.allowed ? "Allowed to crawl" : "Blocked from crawling"}</div><div className="mt-1 break-all font-mono text-[11px]">{routeTest.normalizedPath}</div><p className="mt-2 text-xs leading-5 opacity-90">{routeTest.reason}</p>{routeTest.matchedAgents.length > 0 && <div className="mt-2 text-[11px] opacity-80">Matched agent: {routeTest.matchedAgents.join(", ")}</div>}</div><p className="text-[10px] leading-4 text-[var(--color-text-tertiary)]">This is a local policy preview for common matching behavior. Confirm critical changes with the target search engine’s tester and production logs.</p></section>
