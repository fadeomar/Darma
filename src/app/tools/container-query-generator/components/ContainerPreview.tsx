        ...
        <div className="overflow-auto rounded-[var(--radius-lg)] border border-dashed border-[var(--color-border)] bg-[var(--color-surface)] p-4">
          <div className={cn("mx-auto rounded-[var(--radius-lg)] p-3 transition-all", state.showContainerOutline && "border-2 border-dashed border-[var(--color-accent)]/50")} style={{ width: state.previewWidth, maxWidth: "100%" }}>
            ...
            <article className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-strong)] p-4 shadow-sm transition-all" style={ruleStyles(activeBreakpoints, state.componentClassName)}>
              <div className={cn("rounded-[var(--radius-sm)] bg-[var(--color-accent)]/15 p-8 text-center text-sm font-bold text-[var(--color-accent-text)]", state.previewMode === "dashboard" && "min-h-28", state.previewMode === "product" && "bg-gradient-to-br from-orange-100 to-pink-100", state.previewMode === "article" && "bg-gradient-to-br from-sky-100 to-indigo-100")}>
...
function Metric({ label, value }: { label: string; value: string | number }) {
  return <div className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] p-3"><div className="text-xs font-bold uppercase tracking-wide text-[var(--color-text-soft)]">{label}</div><div className="mt-1 truncate text-sm font-black text-[var(--color-text)]">{value}</div></div>;
}
...
function ActiveRules({ activeBreakpoints }: { activeBreakpoints: ContainerBreakpoint[] }) {
  ...
  return (
    <div className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
      ...
      {rules.length ? <div className="grid gap-2 sm:grid-cols-2">{rules.slice(0, 8).map(({ breakpoint, rule }) => <div key={`${breakpoint.id}-${rule.id}`} className="min-w-0 rounded-[var(--radius-sm)] bg-[var(--color-bg-soft)] p-2 font-mono text-xs"><div className="truncate text-[var(--color-text-soft)]">{breakpoint.name} · {rule.selector}</div><div className="truncate font-bold text-[var(--color-text)]">{rule.property}: {rule.value}</div></div>)}</div> : <p className="text-xs text-[var(--color-text-soft)]">No custom breakpoint is active at this width.</p>}
    </div>
  );
}
